...
- Agentic runs (the target runs its own multi-step tools) need a LARGE `--idle-ms` (12000+) **without `--extract`**: agents go quiet between steps and a short idle window truncates the capture mid-run.
- **Completion under `--extract` is gated on the closing sentinel, not on silence** (`WrapperConfig::idle_gate: Option<IdleGate>`). A settled screen is only a BOUNDED fallback: `--extract-structural` accepts it after `--extract-grace-ms` (default `min(max(4 * idle, 30s), timeout/2)`), strict `--extract` never does. Silence is a guess about model latency and ratcheting `--idle-ms` never converges (#68); the model already emits an unambiguous done token. The gate is `take()`n for the pre-typing readiness wait and restored after (#53) — needle and grace therefore live in ONE field so the grace is suspended with it.
- The grace is measured from the LAST CONTENT CHANGE, so capping its value does not bound the wait: a reply whose last chunk lands late has less than a grace of budget left. `wait_until_idle` therefore also accepts a settled screen once `start.elapsed() + idle_silence >= exec_timeout` — that budget bound, not the `timeout/2` cap, is what keeps #55 from coming back (and it holds for an explicit oversized `--extract-grace-ms` too). Do not "simplify" it away.
- **Sentinel pairs are per-COMMAND, not per-run** (`sentinels(seq)`): the closing marker completes the reply wait, so a shared pair leaves command N+1's gate open on command N's marker — still in the transcript — before the reply exists. `print_capture` extracts between the LAST command's pair. `Wrapper::arm_idle_gate()` (called by `run_command`) is the library-side backstop for a caller that reuses a needle: it requires a needle occurrence ABOVE the count seen at arm time. It is best-effort by construction — a repainting emulator can erase and re-render the same marker inside one read — so do not rely on it instead of unique markers.
- TUI readiness (`Wrapper::saw_output`) is SESSION-sticky, not per-wait: the question is "has this target ever rendered", because a target that has just answered and is quiet at its prompt has no reason to emit anything during the next command's pre-typing readiness wait. Making it per-call strands every command after the first (readiness burns `exec_timeout`, the prompt is never typed, run ends `124` with an empty capture) — pinned by `two_commands_complete_against_a_target_silent_between_replies`.
...
- The `--no-jitter` burst path is best-effort for large prompts only: above a conservative size guardrail (`BURST_MAX_BYTES` in `wrapper.rs`) `send_burst` returns a loud error instead of risking silent mis-delivery. That guardrail is a precautionary POLICY line, not a proven boundary — the #60 burst failure is intermittent and prompt-shape dependent (it can occur below the line too), so the guard does not by itself guarantee correct delivery. Large prompts must use `--paste-input` (atomic bracketed paste), the deterministic delivery path and the real fix for large-prompt callers — do not raise the guardrail to force big prompts through burst.
